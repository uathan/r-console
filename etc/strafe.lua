script = game:GetObjects("rbxassetid://11262617906")[1]
script.Parent = game:service"Players".LocalPlayer.Character
...
if Humanoid then balls =Humanoid:FindFirstChildOfClass("Animator") else balls = nil end

local animator =  balls
...
local function blend2D(targetVelo, targetSpeed)
    ...
    for n,v in pairs(locomotionMap) do
        ...
        if (h[n] / sum > 0.1) then
            sum2 = sum2 + h[n]
            weightedVeloX = weightedVeloX + h[n] * v.lv.x
            weightedVeloY = weightedVeloY + h[n] * v.lv.y
        else
            h[n] = 0.0
        end
    end
